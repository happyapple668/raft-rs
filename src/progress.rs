...
impl ProgressSet {
    ...
    pub fn promote_learner(&mut self, id: u64) -> Result<(), Error> {
        let is_learner = self.has_learner(id);
        let is_voter = self.has_voter(id);
    ...
    //
    // TODO: Accepting the `id` is explictly a choice for Joint Consensus later.
    // TODO: Remove the `id` field. It's only for logs.
    pub fn candidacy_status<'a>(
        &self,
        id: u64,
        votes: impl IntoIterator<Item = (&'a u64, &'a bool)>,
    ) -> CandidacyStatus {
        let (accepted, total) = votes.into_iter().fold(
            (0, 0),
            |(mut accepted, mut total), (_, nominated)| {
                if *nominated {
                    accepted += 1;
                }
                total += 1;
        ...
        let quorum = self.num_voters() / 2 + 1;
    ...
    pub fn quorum_recently_active(&mut self, perspective_of: u64) -> bool {
        let learners = self.learner_ids().cloned().collect::<FxHashSet<_>>();
        let active = self
            .progress
            .iter_mut()
